...
function getGroupTabY(display, index) {
  // Stack group tabs below the main tab with a small gap
  const mainTabBottom = 80 + TAB_H + 8;
  return display.workArea.y + mainTabBottom + index * (GTAB_H + 6);
}
...
function openApp(navigateToGroup) {
  if (isAppOpen && appWindow) {
    ...
    appWindow.focus();
    if (navigateToGroup) {
      appWindow.webContents.send('navigate-group', navigateToGroup);
    }
  } else {
...
function rebuildGroupTabs(favourites) {
  // Close all existing group tabs
  Object.keys(groupTabWindows).forEach(closeGroupTab);
  // Recreate from current favourites list
  favourites.forEach((fav, i) => {
    createGroupTab(fav.name, fav.color, i);
  });
}
...
ipcMain.on('open-group', (event, groupName) => {
  openApp(groupName);
});
...
ipcMain.on('save-data', (event, data) => {
  try {
    ...
    // Rebuild group tabs whenever data is saved (favourites may have changed)
...
app.whenReady().then(() => {
  ...
  try {
    ...
    if (fs.existsSync(dataPath)) {
      ...
      if (saved.favourites && saved.favourites.length) {
        rebuildGroupTabs(saved.favourites);
      }
    }
  } catch(e) {}
