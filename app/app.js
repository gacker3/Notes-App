...
function openGroupNameModal(selectedNotes) {
  ...
  // Show existing groups as quick-pick chips
...
function syncTodoUI() {
  ...
  document.querySelector('.e-dates').style.display = isTodo ? 'none' : 'flex';
}
...
function openEditor(id) {
  ...
  // Load date fields (day, month, year)
  document.getElementById('eDay').value = n ? (n.day || '') : '';
  document.getElementById('eMonth').value = n ? (n.month || '') : '';
  document.getElementById('eYear').value = n ? (n.year || '') : '';
...
document.getElementById('eBody').addEventListener('keydown', e => {
  // Tab key - focus on eDay
  if (e.key === 'Tab') {
    e.preventDefault();
    document.getElementById('eDay').focus();
  }
  // Allow Shift+Enter (Windows) or Cmd+Enter (Mac) to insert a new line
...
document.addEventListener('keydown', e => {
  ...
  if (document.getElementById('overlay').classList.contains('open')) {
    ...
    if (e.key==='Enter' && !e.shiftKey && !e.metaKey) saveNote();
    return;
  }
  ...
  if (e.shiftKey && e.key==='A') {
    ...
    if (old) { snapshot('archive note'); old.archived=true; if(S.mode==='float') renderFloat(); toast('Archived: '+old.title.slice(0,28)); }
