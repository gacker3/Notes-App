...
function renderFloat() {
  ...
  Object.entries(byGroup).forEach(([g, notes]) => {
    ...
    const LABEL_H = 22;
    const bg = div('group-bubble' + (S.favourites.some(f => f.name === g) ? ' favourite' : ''));
    bg.dataset.group = g;
    ...
    // star favourite button (visible on hover, left of three-dot)
    const isFavNow = S.favourites.some(f => f.name === g);
    const starBtn = div('group-star-btn');
    starBtn.textContent = isFavNow ? '★' : '☆';
    starBtn.title = isFavNow ? 'Remove favourite' : 'Favourite as tab';
    starBtn.classList.toggle('active', isFavNow);
    ...
    menuBtn.addEventListener('click', e => {
      ...
      const favColor = isFav ? (S.favourites.find(f => f.name === g).color) : '#c8e6c9';

      popup.innerHTML = `
        <div class="gm-item" id="gm-favourite">
          ${isFav ? '★ Unfavourite' : '☆ Favourite as tab'}
        </div>
        ${isFav ? `<div class="gm-item gm-colors" id="gm-colors">
      ...
      document.getElementById('gm-favourite').onclick = e => {
        ...
        if (isFav) {
          ...
          S.favourites.push({ name: g, color: favColor });
        }
        persist();
        if (window.electronAPI) window.electronAPI.syncFavourites(S.favourites);
        closeGroupMenu();
        renderFloat();
        toast(isFav ? `"${g}" removed from tabs` : `"${g}" added as tab`);
      };
      ...
      popup.querySelectorAll('.gm-color-dot').forEach(dot => {
        dot.addEventListener('click', e => {
          ...
          if (fav) { fav.color = color; persist(); if (window.electronAPI) window.electronAPI.syncFavourites(S.favourites); }
...
function persist() {
  if (window.electronAPI) {
    window.electronAPI.saveData({ notes: S.notes, groups: S.groups, nextId, favourites: S.favourites });
  }
}
...
if (window.electronAPI) {
  ...
  // Listen for group tab clicks — navigate to that group in finder view
